...
async function ensureUserAppDeployment(
  ...
  try {
    ...
    // Kubernetes client-node exposes the HTTP status as err.code.
    // 404 means the Deployment does not exist yet and should be created.
    if (err.code !== 404) {
    ...
    try {
      ...
      // Kubernetes client-node exposes the HTTP status as createErr.code.
  ...
  try {
    ...
    // Kubernetes client-node exposes the HTTP status as code.
